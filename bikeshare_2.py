...
def get_filters():
    ...
    # get user input for city (chicago and new york city and washington). HINT: Use a while loop to handle invalid inputs
    while True:
        city=str(input('Select one city from Chicago and New York City and Washington. \n')).lower()
        if city not in cities:
            print('Selected City is not a valid one. Please enter a valid city name')
        else: 
            break

    # get user input for month (all, january, february, march, ... , june)
    while True:
        month=str(input('If want to filter by month, then type out the month. If not, type in all\n')).title()
        if month not in months:
            print('valid month name not selected')
        else:
            break

    # get user input for day of week (all, monday, tuesday, ... sunday)
    while True:
        day=str(input('If want to filter by day, then type out the day. If not, type in all\n')).title()
        if day not in days:
